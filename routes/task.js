...
router.post("/task/:id", async (req, res, callback) => {
  ...
  unreq.end(async (response) => {
    ...
    if (response.error) {
      ...
      res.send({
        error: "sorry something wrong on our side, please try again later!",
      });
    }
    ...
    setTimeout(
      function (res) {
        var rereq = unirest(
          "GET",
          "https://judge0.p.rapidapi.com/submissions/" + response.body.token
        );

        rereq.headers({
          "x-rapidapi-host": "judge0.p.rapidapi.com",
          "x-rapidapi-key": "1f65ff877msh1db5c2db38010f1p1a2538jsn5dc333e8e899",
          useQueryString: true,
        });
